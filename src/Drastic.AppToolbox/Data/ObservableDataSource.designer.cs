...
public partial class ObservableDataSource<T> : IObservableDataSource<T>
{
    ...
    {
        if (!this.observers.Any(a => a.IsAlive && a.Target is not null && a.Target.Equals(observer)))
        {
    ...
    {
        var l = this.observers.Where(a => a.Target is not null && a.Target.Equals(observer)).ToList();
